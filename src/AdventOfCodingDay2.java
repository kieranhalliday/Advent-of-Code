public class AdventOfCodingDay2 {

    static Integer arr[][] = {{493, 458, 321, 120, 49, 432, 433, 92, 54, 452, 41, 461, 388, 409, 263, 58},
    ...
    private static int partTwoB() {
        int sum = 0;
        for (Integer[] anArr : arr) {
            for (int i = 0; i < anArr.length - 1; i++) {
                for (int j = i + 1; j < anArr.length; j++) {
                    if (anArr[i] % anArr[j] == 0) {
                        sum += anArr[i] / anArr[j];
                    } else if (anArr[j] % anArr[i] == 0) {
                        sum += anArr[j] / anArr[i];
                    }
                }
            }
